...
function minifyJs(src) {
  ...
  if (fs.existsSync(bin)) {
    ...
    try {
      ...
      /* passes=3: the compressor is a fixed-point pass and one round leaves
         work on the table — inlining exposes folds, folding exposes more
         inlining. Worth about 40 characters here, which is not much until you
         are 138 under Webflow's cap. Semantics are unchanged, and the suite
         drives this output rather than the source. */
      const out = execFileSync(bin,
        [tmp, '--compress', 'passes=3', '--mangle', '--format', 'quote_style=1'],
...
function minifyCss(css) {
  return css
    ...
    .replace(/:\s*0px\b/g, ':0')
    /* #ffffff -> #fff. Only the repeating ones, which is the only case where
       the short form is the same colour. */
...
/* Every character here comes out of the 50,000 Webflow allows, so the banner
   says the one thing a person opening this Embed needs — where to edit — and
   stops. The long version lives in the README, which costs nothing. */
const BANNER =
  '<!-- Upserve demo form. Generated: edit src/webflow-embed.html, not this. -->\n';
...
/* `npm run copy` used to be `pbcopy < webflow/embed.html`, which dies with a
   shell error the day the form crosses the cap and that file stops existing.
   Doing it here means the copy step knows which shape the build just emitted,
   and can say so. */
if (process.argv.includes('--copy')) {
  ...
  try {
    ...
    console.warn('  warn: could not reach pbcopy — paste ' +
                 path.relative(ROOT, pick) + ' by hand.');
  }
}
